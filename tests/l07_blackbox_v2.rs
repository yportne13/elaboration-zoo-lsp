//! L07_sum_type 黑盒测试套件 v2（第二轮独立攻击面）。
//!
//! 与 `tests/l07_blackbox.rs`（49 例：48 可跑 + 1 个 `--ignored` 探针，已覆盖
...
//!
//! 运行：`cargo test --test l07_blackbox_v2`；
//! 格式探针：`cargo test --test l07_blackbox_v2 v2_probe -- --ignored --nocapture`。

use elaboration_zoo_lsp::L07_sum_type::{run, FILE_IO_LOCK};
