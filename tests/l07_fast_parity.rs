//! L07_sum_type 双 oracle 互检套件（参考版 `run` ↔ 性能版 `bump_spine_iter::
//! run_fast`）。`#[path]` 只编 list.rs / parser_lib.rs / L07_sum_type/mod.rs
...
fn parity_demo_src() {
    ...
    assert_parity(L07_sum_type::DEMO_SRC);
}
...
fn deep_workloads_parity() {
    ...
    let basic = {
        ...
        std::thread::Builder::new()
            .stack_size(256 * 1024 * 1024)
            .spawn(move || L07_sum_type::bench_check(&ast))
            .unwrap()
            .join()
            .unwrap()
    };
    ...
    let basic_nf = {
        ...
        std::thread::Builder::new()
            .stack_size(256 * 1024 * 1024)
            .spawn(move || L07_sum_type::bench_check_nf(&ast))
            .unwrap()
            .join()
            .unwrap()
    };
