...
| Bundle | `#[derive(Bundle)]` + `impl IMasterSlave` | SpinalHDL 风格批量赋值；自动命名信号工厂（`create_TypeName`，绑定名前缀）；方向在 `impl IMasterSlave` 的 `asMaster` 中声明（struct 不带 in()/out() 标记），`asSlave` 自动翻转 |
