...
> 实施记录（阶段 0-2 + I3/I4/X3）：import_map 落地 20/20 测试绿 + 全量 308 回归绿；**G7/G5/G8/G2/G3 已修**；**I4 已修**（inherent 方法不前缀 + `infer_after_prefix`）；**I3b 已修**（trait 名称按 namespace_prefix 解析 + TraitDecl/ImplDecl 方法名统一不前缀，含 `f.getVal` 派发）；**X3 已修**（supertrait 按 namespace_prefix 解析，测试 `supertrait_in_package_inherits_methods`）；**G1a' 实测降级**
...
| I4 | **固有 impl 跨文件与 package 内**：`cxt.namespace` 条目不随文件同步（跨文件部分仍在）；**package 内同文件已修**——探针确认 `mylib.Foo has no object double` 后修复：`prefix_decl_name` 不再前缀 inherent 方法名（方法按书面名注册进 namespace 集合），方法 def 注册改 `infer_after_prefix`（防 `Foo.double` 二次加前缀成 `mylib.Foo.double`）。测试 `inherent_method_in_package_dispatches`。**跨文件部分待 Phase 4 同步 `cxt.namespace`** | `elaboration.rs:1168` |
| I5 | **跨文件 trait 实例不可用**：`trait_solver`（Synth）+ `trait_definition/assoc_defaults/symbol_table` 不随文件同步 | `mod.rs:770-802` |
...
| L2 | import 上下文补全缺失：`import mylib.` / `import mylib.{` 无补全 | `lib.rs:1376-1456` |
