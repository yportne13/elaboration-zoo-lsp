...
| I4 | **固有 impl 跨文件与 package 内 → 均已完成**：package 内（方法不前缀 + `infer_after_prefix`，测试 `inherent_method_in_package_dispatches`）；**跨文件**——`elaborate` 写回把本文件的 inherent namespace 条目合并进全局 `cxt.namespace`（按类型值指针去重，`file_namespace_regs` 跟踪，编辑/关闭时移除），import 文件的 `x.method` 可派发（测试 `cross_file_inherent_method_dispatches`）。**namespace 在 Cxt 上同步，避开 infer 写回陷阱**。**编辑去重修复**：重展开时 `local_cxt.namespace` 克隆自全局（含本文件旧条目），若不移除则写回合并会把新旧条目都加入 → 重复 → `x.method` ambiguous（HDL `.tree` 复现）；已在 elaborate 开始时从 local 滤掉旧注册（测试 `editing_module_file_keeps_tree_trait`） | `elaboration.rs:1168` + `lib.rs` 写回 |
| I5 | 跨文件 trait 实例同步（**未做，实测遇核心检查器 bug**）：实现并验证了同步方案（trait_decls/trait_instances 记录、merge/remove、file_trait_regs）——trait **声明**同步可用；**实例**同步触发 `def use` 类型检查时的 meta 池 OOB（`force` 悬空 `Flex(1628)`，len 1627）。**精确定位**：1628 是 B 自己的 `U(0)` 类型 meta；B 派发期间池从 ~1630 截断到 ~1625，但全部 8 处已知 meta 突变点（3 处 truncate + 5 处 snapshot-restore）**全部插桩均未触发**——存在未记录的 meta 池缩减路径（检查器/solver 深处）。同步数据本身干净（实例参数、A 的 decl 均无 Flex）。**结论：核心检查器存在未定位的 meta 池管理 bug，需专项调查**（从 solve_multi_trait/solve_trait 调用间隙的池变化入手） | `mod.rs:770-802` + `elaboration.rs` trait_wrap |
