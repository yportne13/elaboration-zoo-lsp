...
| I5 | 跨文件 trait 实例同步（**未做，实测遇前置 bug**）：实现"文件内注册记录（trait_decls/trait_instances 字段）→ merge/remove 方法 + file_trait_regs 跟踪"方案并验证：**trait 声明同步可用**（B 能识别跨文件 trait，缺实例时报 `has no object` 而非 OOB）；**实例同步触发预先存在的 trait-solver meta 池 OOB**（B 派发时 `index 1628 vs len 1627`，force 悬空 Flex）。已排查：实例参数封闭（Sum 无 Flex）、绕过 head_index 仍 OOB、prelude 污染已清——**非同步代码本身，是 trait_wrap 的 snapshot/rollback meta 泄漏**（`solve_multi_trait`/`fresh_meta` 路径），单文件同构用例不触发、跨文件同步后可达。**前置条件：先修 trait-solver 的 meta 管理（快照/回滚不泄漏悬空 Flex），再启 I5** | `mod.rs:770-802` + `elaboration.rs` trait_wrap |
