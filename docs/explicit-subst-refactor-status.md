...
放大，**两轮修复后仍未达验收，已回退至 HEAD**。证据与补丁存档见
`docs/wip/`（README 载最终根因、复现数字与接续建议）。

根因（最终结论）：

- 移植后 calc/prelude 路径构造出**巨型卡住应用**（单次顶层 quote 扇出
  77,811 节点 / 祖先链 12,287 层，形状 `Flex=1, Rigid=4, VSub=77,806`），
  `calc_two_step`（226 字节）触发 force 2.06e7 / quote 1.11e7 次。
- 参考版 quote 无记忆化且 `quote_sp` 递归 ⇒ 8MB 测试栈爆栈；迭代化 +
  记忆化后可完成但 198s（移植前 5.34s）。
- **语义回归**：`calc_err_*` 两条报错文案改变（移植前 `name not in
  scope: calc`），parser/macros/preprocess 逐字节一致 ⇒ 差异在
  elaboration 层的报错走向。

即"为何 calc 展开在 σ 机制下累积出 7.8 万实参的卡住应用"尚未定位——这是
与 L07–L12 的本质差异（那些层的 prelude 规模未暴露该病理）。**L13 保持
