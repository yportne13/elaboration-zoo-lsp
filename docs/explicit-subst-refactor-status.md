...
| L13（生产版） | ❌ 未落地（五轮尝试 + 5-agent 评审轮；GADT 覆盖回归参考版已修，余项见清单） | 见 `docs/wip/` |
...
- **GADT 覆盖回归已修复（第五轮，参考版）**：可达性探测用的是精化前构建
  ...
  类型；**评审轮补充：两个内层精化块的 `SpecSolve.acc` 改以入臂 σ 起种子**
  （否则多臂异 σ 子树里已解变量被再解）。3 个 GADT 测试转绿且无新增失败。
- **评审轮（2026-09-17，5 子 agent）**：正确性（σ-ABA、fuel=0 缓存洞）、
  性能（inlay 总闸）、测试（蓝图 §9.5 钉子测试补入，实测暴露 fn 型索引槽
  解析应用为 HEAD 既有缺口，标 ignore 留档）、文档（数字对账）、架构
  （偏差清单补录）——处置表见 `docs/wip/README.md` 评审轮一节。
