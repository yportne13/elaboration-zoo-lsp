...
- **L11：已完成**（`6ae12a6`）——参考版 + 孪生同步重写；比值（两窗口一致）：
  参考版 2.2–2.3×、孪生 8.5×；孪生/参考从 0.3×（落后）变为 **1.02–1.31×**
  （矩阵重跑 `docs/bench-matrix-2026-09-18.md`）。移植当时 A/B 报的 "~3×"
  是该窗口参考版被测偏慢所致，已订正。
- **L12：已完成**（`45330e6`）——参考版 4.8×、孪生 8.9×；孪生/参考
  0.98–1.24×（同前订正口径）。
- **L10：已完成**（`4e97743`）——参考版 3.0×、孪生 5.1×；孪生/参考
  0.8× → 1.28×。
- **L09：已完成**（`09b9ee8`）——参考版 3.8×（2.215→0.576 ms，残差是
  L09 参考引擎地板：非 match 负载本就系统性慢 L10 参考 2.5–4.5×）、孪生
  **9.6×**（0.469→0.049 ms @k=11，与 L10–L12 同档）；其余负载 0.96–1.07×。
  **顺带修复**：`enum` 负载两侧都能跑了且快 10×（basic 14.05→1.37 ms @k=9，
...
5. **`PatternDetail` 是另一套形态**（L13 独有的三字段版）：
   `Con(idx, name, subs)` / `Any(var_name, param_name, icit)` / `Bind(name)`——
   `idx` 是命名空间索引、`Any` 带**隐式参数名**。运行时 `eval_aux` 消费的就是
   这个形状，所以 `walk_pat` 必须按它构造（不是 L11/L12 的 `Con(name, subs)` /
...
7. **错误是"收集"而非"短路"**：`Compiler::compile` 返回
   ...
   语义（签名与 `errors` 字段都得留着）。
8. **L09**：其孪生本就比参考版快 2.3×（`match`），优先级最低；适配点预计介于
