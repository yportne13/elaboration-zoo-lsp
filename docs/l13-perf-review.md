...
**2026-09 实测结论：不值得做（关闭）。** 在 `eval_inner` 的 `Tm::Var` 分支临时
插桩统计真实负载（`l13bench` 参考版）：

- `prelude-core`（134 decls）：55,410 次查找，总步数 95,684，**平均深度 1.73**，
  最大 15。
- `examples-hdl`（143 decls）：332,220 次查找，总步数 573,888，**平均 1.73**，
  最大 15。
- 合成负载 `church`（k=13）：254,042 次查找，**平均 0**；`enum`：平均 1.32，
  最大 8。

即变量引用几乎总落在最近 1–2 个绑定上（印证"最近定义的变量访问最多"），原文
假设的深度 30–50 未出现。按 `examples-hdl` 计，全部 Var 步进约 57 万次，相对
约 25ms 的总检查时间占比 ~1–2%；就算把查找降到 O(1)，收益也在噪声内。`List`
的 O(1) prepend / 共享尾反而比数组的 O(depth) 拷贝更适合这种浅环境。**不改环境
表示。**
