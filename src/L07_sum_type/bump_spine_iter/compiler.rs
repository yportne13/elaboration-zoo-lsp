...
//! `is_catch_all`/`probe_accessible`/`unify_indices`）、下钻 `walk_con`。
//! 原 bump_spine_iter.rs 的 "模式匹配编译" 节，逐行搬运（2026-09-19 拆分）。
...
use super::machine::{bind_slots, Cxt, Machine};
use super::prim::UNIFY_FUEL;
use super::subst::{wrap_sub, SpecSolve, SubstV};
use super::syntax::{SumDataV, Tm, V, XCell, v_lvl, v_lvl_of, v_pi_of, v_tag, v_xcell, v_xcell_of};
...
impl<'a> Compiler<'a> {
    ...
    /// 上不可能有 `cons`）= absurd。`lvl` 显式穿参（参考版同款）：顶层
    /// 探测传入口 `cxt.lvl`；嵌套位置的延迟探测传记账时的臂内层级。
    fn probe_accessible(
        mach: &mut Machine,
        bump: &'a Bump,
        cxt: &Cxt<'a>,
    ...
    fn unify_indices(
        ...
        for (a, b) in hp.iter().zip(rp.iter()) {
            if !mach.unify_spec(bump, &decl, lvl, a.val, b.val, Some(spec)) {
