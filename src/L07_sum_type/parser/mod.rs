...
where
    P: Parser<&'b [TokenNode<'a>], O>,
{
    (kw(LCurly), kw(EndLine).option(), p, kw(EndLine).option(), kw(RCurly)).map(|c| c.2)
}
    ...
    (
        ...
        brace(
            (
                ...
                kw(T![=>]),
                p_raw,
            ).map(|(_, pattern, _, body)| (pattern, body)).many0_sep(kw(EndLine))
        )
    )
...
fn p_raw<'a: 'b, 'b>(input: &'b [TokenNode<'a>]) -> Option<(&'b [TokenNode<'a>], Raw)> {
    p_lam.or(p_let).or(p_pi).or(fun_or_spine).or(p_match).parse(input)
}
...
fn p_def<'a: 'b, 'b>(input: &'b [TokenNode<'a>]) -> Option<(&'b [TokenNode<'a>], Decl)> {
    (
        ...
        .map(|(_, name, params, ret, _, _, body)| {
            Decl::Def {
                name,
                params,
...
fn p_enum<'a: 'b, 'b>(input: &'b [TokenNode<'a>]) -> Option<(&'b [TokenNode<'a>], Decl)> {
    (
        ...
        brace(
            (
                string(Ident),
                paren(p_raw.many0_sep(kw(T![,]))).option().map(|x| x.unwrap_or_default())
