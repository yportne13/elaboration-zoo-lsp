...
//!                                    [--only basic,fast]
//!                                    [--workload church|natadd|strchain|match|enum|struct|macro|universe|traitchain|all]
//! ```
...
fn run(cli: Cli) {
    ...
    let workloads: &[&str] = match cli.workload.as_str() {
        ...
        // a(N, s, b(N, s, z)) 后两版一致通过（见 L11 twin church_src 注）
        _ => &[
            "church", "natadd", "strchain", "match", "enum", "struct", "macro", "universe",
            "traitchain",
        ],
    };
