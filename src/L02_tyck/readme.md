...
2. **conv 连续链内联环**（conv 再 ~1.1–1.2×）。`(2,2)` 中性比较原来是每
   ...
   双方 `.a` 仍为 spine 就地推进，只有真正待比较的子对才入工作表。位相等
   剪枝受 `L02_NO_BITEQ` 统辖（消融仍可关掉全部捷径）。
   **修订（2026-09-02）**：原「入口处两侧连续链先比 `len`，长度不等 fail-fast」
   的守卫已**移除**——其论证「条目数即归一化后的应用个数」只覆盖了 f 分量
   非闭包，漏了链 base 的 `a` 可以是闭包：η 把应用吸收进 λ 体，
   `P (h y)`（len 2）与 `P (\x. h y x)`（len 1）βη 等价却被误判不等
   （黑盒 `conv_eta_absorption_accepted_by_both` 钉住）。L02 的 conv 无求解
   副作用，内联环沿 `.a` 下钻的比较集与参考版逐层 `(f,a)` 分解相同、结论
   与派发序无关，故仅去守卫、保留内联环（L03/L04 因 unify 带求解副作用，
   改成了 L05 同款单步派发，见其 readme）。
...
4. **否定一个轴前先确认负载覆盖了它的触发条件**：第二轮暂缓 quote 侧
   ...
   属性，不是机制的属性。补一个造命中场景的负载（dup 族）后 1.9×/3.6×
   立即兑现。反过来，β(clo,arg) eval 侧记忆化的否决经住了复验：NbE 的
   CBV 只强制到 WHNF，昂贵的整链展开 β 键在任意负载下都不重复（见上节
   call-by-need 注记）。
5. **快速路径的健全性论证要枚举全部等价来源，η 不是结构同形的**：
   conv 链长 fail-fast 的论证只覆盖了「f 分量恒非闭包」，漏了链 base 的
   `a` 可以是闭包——η 展开把应用吸收进 λ 体，条目数 ≠ 归一化应用数，良型
   程序被误杀（教训 3「快速路径不能是正确性的唯一依托」的同族：那次是位
   相等兜底缺显式分支，这次是剪枝条件本身不成立）。L03/L04 的同类守卫还
   叠加了 meta 实参的 len 虚增机制（L04 当初只记录了这一条）。教训：给
   「必不等」类捷径写论证时，先把 conv/unify 的全部正例来源（β、η、meta
   求解、共享句柄）逐条对照剪枝条件。
...
| ——（L02 后续提速） | conv 连续链内联环（沿 `.a` 前进免 worksheet 往返；入口长度 fail-fast 已移除，见「后续提速」节修订与教训 5） |
...
- 「深度无上限」只对**机器核**成立（bench 的 check/nf 全迭代）：`fast` 的
  输出路径同样受限——`main_with` 的 pretty 打印（`pretty_tm::go` 递归）与
  导出 `Box` 树的递归 Drop 都要栈（黑盒深链用例即挂 512 MB 大栈跑）。
  两版共享该路径，非分歧项，但「fast 深度无上限」的表述要按此理解。
...
- **quote 记忆化默认口径的分层分歧（刻意）**：L03 起把 quote_memo 设为
  `Tycker::run` 的生产默认（`run_no_memo` 为消融对照，L04+ 同款）；本章
  `run` 默认普通口径、`run_memo` 为记忆化口径——两口径输出逐字节一致
  （`church_memo_matches_basic` 等互检），分歧只在默认值。不回灌的理由：
  对齐需同步翻转 `src/bin/l02bench.rs` 的 `fast`/`fast_memo` 口径行语义
  （bin 不属本章所有）；机制锚点见上文 dup 节与 L03 readme「实测结果」。
- **环境机制的低层朴素形态（刻意，下沉评估已裁决）**：L02 性能版的
  `Raw::Var` 解析沿 `types` 链线性走查、`nth` 沿 `EnvCons` 链取值——没有
  L03 的 name_map（O(1) 名字解析）与 defs 平坦区（复合环境，
  6efa7dc/ccf943d/985390b/a483331）。现有负载（church/conv/dup 族）的
  变量引用都落在链头附近（O(1)），上文各表实测严格线性（每翻倍 ×2），
  本章负载面与文档均不承诺「长 let 链引用最老名字」形态，故该二次方
  只是潜伏上界、不构成缺陷。**不下沉的理由（教学定位）**：L02 是引入
  朴素环境的基层章节，L03 的两项机制是在**完整继承 L02** 之上叠加的
  性能演进——继承判据「下一章完整继承上一章」约束的是向上一侧的完备性
  （L03 含 L02 全部代码 + 已文档化演进，成立），不要求性能机制向低层
  对称回灌；回灌属 ~300–500 行 × 双版本（参考版 Cxt + 快版 Machine）
  的机制移植，含 trail 撤销轨迹与 tip 判定（`binds.is_none()`，必须随
  初版带上）等紧邻 unsafe 打包字的细节，且需 l02bench 新增 chain 负载
  做专项验证——按评审规范「需要专项验证的高危重构只记录、不盲改」
  暂缓。重启条件：若 L02 负载面将来出现链式引用形态（如 l02bench 新增
  chain 负载），以 L03 readme「名字解析 O(1)」节与上述 commits 为移植
  蓝本重开评估。
