            ...
            Raw::App(t, u) => {
                ...
                let (a, b) = match self.force(&tty) {
                    ...
                    tty => {
                        ...
                        // 实参序 (合成Π, tty) 系上游 03 的 unifyCatch 次序；
                        // 上游 04/05 是 (tty, 合成Π)——各层各自忠实，只影响
...
/// `goBDS`：`?m n_外 … n_内`（ns 与 bds 自**内层**平行推进——ns 外→内
/// 存放，故取 `split_last` 配 bds 头；`Defined` 跳名）。
fn go_bds(p: usize, ns: &[String], m: u32, bds: &List<BD>, out: &mut String) {
    match (ns.split_last(), bds.head()) {
        (None, None) => out.push_str(&format!("?{m}")),
        (Some((n, ns_prefix)), Some(BD::Bound)) => {
            let paren = APPP < p;
            if paren {
                out.push('(');
            }
            go_bds(APPP, ns_prefix, m, &bds.tail(), out);
            out.push(' ');
            out.push_str(n);
            if paren {
                out.push(')');
        ...
        (Some((_, ns_prefix)), Some(BD::Defined)) => go_bds(APPP, ns_prefix, m, &bds.tail(), out),
...
"#
    ...
    fn fast_impl_matches_basic_on_examples() {
        for (name, src) in [
            ...
            // 类型洞的 AppBds 配对把 ?0 应用到 q0 的值（U）而非 u，错位值
            // 流进 solve 后 invert 遇非变量实参必败 → 误报 Cannot unify
            // （参考版正确解出 ?0 := λx1. U）。修复前此用例三模式全分歧。
            ("λ 体内 define 错位回归",
