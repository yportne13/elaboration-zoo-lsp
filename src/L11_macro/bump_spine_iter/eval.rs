...
//! 迭代 eval（`W`/`eval_iter`：decl 表查名、带类型 Prim 的五路分派、投影的
//! panic 语义、match 的编译期选择与卡住停等）。原 bump_spine_iter.rs 的
//! "运行时分支选择" + "eval" 两节，逐行搬运（2026-09-23 拆分）。
...
use super::env::{env_ext, env_nth, CloCell, Decls, Env, PiCell};
use super::force::{force, vapp1};
use super::spine::{lit_of, meta_val_of, MetaEntry, Spine};
use super::syntax::{
    PrCons, PrimId, SumDataT, SumDataV, SumParamT, SumParamV, Tm, V, XCell, v_clo,
