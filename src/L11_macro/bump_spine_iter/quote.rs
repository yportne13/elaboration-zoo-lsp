//! quote：任务栈 quote（`QJob`/`quote_iter` + `QuoteMemo` 记忆化；流式右链，
//! 卡住 match 分支体在 declb 存根表下重求值）。原 bump_spine_iter.rs 的
//! "quote" 节，逐行搬运（2026-09-23 拆分）。
...
use super::env::{env_ext, Decls, Env, PiCell};
