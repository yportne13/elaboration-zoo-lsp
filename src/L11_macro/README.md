...
动机是矩阵复测发现 `match` 负载上孪生反而比参考版慢 3×（决策树的逐节点
上下文克隆与重复探测）。实测（`l11bench --workload match`，同窗口交错，min）：

| 口径 | k=11 前 | k=11 后 | 倍数 |
|---|---|---|---|
| 参考版 basic | 1.250 ms | 0.542 ms | 2.3× |
| 孪生 fast_ss | 1.524 ms | **0.180 ms** | **8.5×** |
| 孪生 fast_run | 1.575 ms | 0.228 ms | 6.9× |

孪生/参考的相对关系从 0.3×（更慢）变为 ~3×（更快），与 L07/L08 同型。其余负载
（`strchain`/`struct`/`macro`/`universe`/`traitchain`/`natadd`）0.95–1.14×，无回归。
