...
use super::{
    Closure, Cxt, DeclTm, Error, Infer, Lvl, Subst, Tm, VTy, Val,
    Rc,
...
/// 结束后取走 `acc` 作为新 σ。L11 的可解集 = **任意裸 Rigid**（旧
...
impl Infer {
    ...
    /// 不可再解。
    ///
    /// 与 trait 求解的交互：本函数**只**服务模式方程路径（pattern_match 的
    ...
    pub(crate) fn unify_pm(
        ...
        if !spec.acc.is_empty() {
            t = self.force(&cxt.decl, &Rc::new(wrap_sub(&spec.acc, (*t).clone())));
            t_prime = self.force(&cxt.decl, &Rc::new(wrap_sub(&spec.acc, (*t_prime).clone())));
        }
        match (t.as_ref(), t_prime.as_ref()) {
            ...
            // 单侧裸 Rigid：解入 acc。Flex 解值保持旧 `update_cxt` 的
            // no-op（那时对 Flex 直接 clone、不改写槽）；occurs 环守卫
            // 只扫解值自身结构，失败 = 方程不可满足（分支不可达）。
            ...
            (
                ...
                // Sum 头名字检查（2026-09-18 评审修复，L07 unify 同款）：
                // case_name 不足以判定构造子身份——跨 enum 重名构造子
                // （E1.c / E2.c）是两个不同值，头名不同直接失败；不比值
                // （深递归），只在头名以 Sum 形态可见时比较。
                if name1 == name2 {
                    if let (
                        Val::SumCase { typ: ta, .. },
                        Val::SumCase { typ: tb, .. },
                    ) = (t.as_ref(), t_prime.as_ref())
                    {
                        if let (Val::Sum(na, ..), Val::Sum(nb, ..)) = (ta.as_ref(), tb.as_ref()) {
                            if na.data != nb.data {
                                return Err(Error(t_span.map(|_| "".to_string())));
                            }
                        }
                    ...
                    for (x, y) in d1.iter().zip(d2.iter()) {
                        self.unify_pm(cxt, lvl, &x.1, &y.1, t_span, spec)?;
                    }
                    Ok(())
                } else {
            ...
            (_, _) => self.unify_catch_at(cxt, lvl, &t, &t_prime, t_span),
        }
    }
        ...
        match inferred_type.as_ref() {
            ...
            Val::Flex(m, sp) => {
                ...
                let mty = match self.meta[m.0 as usize] {
                    ...
                    // fuel 耗尽窗口（2026-09-18 评审修复，L07 同款降级故事）：
                    // force 耗尽时把已解 meta 当未解返回，走到这里按失败降级
                    // 而不是 panic
                    _ => {
                        return Err(Error(
                            t_span.map(|_| "check_universe: meta already solved (fuel exhausted)".to_owned()),
                        ))
                    }
                };
        ...
        match t {
                        ...
                        let err_msg = if let Val::Sum(name, params, _, true) = meta_ty.as_ref() {
                            ...
                            if has_flex {
                                ...
                                let pretty_val = |val: &Rc<Val>| {
                                    super::pretty_tm(0, ret_cxt.names(), &self.quote(&ret_cxt.decl, ret_cxt.lvl, val))
                                };
            ...
            Decl::ImplDecl { name, params, trait_name, trait_params, methods, need_create } => {
                ...
                if need_create {
                    ...
                    let typ = self.force(&temp_cxt.decl, &self.eval(&temp_cxt.decl, &temp_cxt.env, &typ))
                        .to_typ()
                    ...
                    for a in trait_params.clone() {
                        ...
                        let a_eval = self.force(&temp_cxt.decl, &self.eval(&temp_cxt.decl, &temp_cxt.env, &a_checked));
                        match a_eval.to_typ() {
    ...
    /// 构造子返回类型良构性（2026-09-18 评审修复，L07 `Infer::check_ctor_wf`
    /// 同款）：实例化构造子类型的全部绑定器后，ret 的 WHNF 必须是
    /// `enum_name` 的 `Sum`，且其隐式参数位逐一等于 telescope 内的 bare
    /// rigid。允许构造子重绑定参数（`p[A,B](a,b) -> Pack[A][B] a b`——使用
    /// 点经特化方程解回枚举参数，v3_multi_index_gadt 钉），拒绝参数位为非
    /// 变量的特化（`c -> Foo[Bool]`）与非本 enum 的 ret（`c -> Nat`）：后者
    /// 向构造子名字空间注入永不匹配任何模式的 phantom 值，对覆盖检查完备
    /// 的 match 在封闭输入上卡死。
    fn check_ctor_wf(
        &mut self,
        cxt: &Cxt,
        ...
        let decl = &cxt.decl;
        let base = cxt.lvl.0;
        let mut ty: Rc<Val> = ctor_vtyp;
        let mut bound = 0u32;
        let ret = loop {
            let tyf = self.force(decl, &ty);
            match tyf.as_ref() {
                Val::Pi(_, _, _, closure) => {
                    let u = Val::vvar(Lvl(base + bound));
                    bound += 1;
                    ty = self.closure_apply(decl, closure, u.into());
                }
                _ => break tyf,
            }
        };
        let ret_sum = self.force(decl, &ret);
        let (sname, sparams) = match ret_sum.as_ref() {
                        ...
                        if cases.len() == 1 {
                            if let Some(case) = cases.first() {
                                if case.data.contains(".mk") {
                                    ...
                                    param.reverse();
                                    // 精化 σ 包裹的类型先 force 推开再剥 Π（旧机制下
                                    // src_names 已被 refresh 物化）
                                    loop {
                                        let tf = self.force(&cxt.decl, &typ);
                                        match tf.as_ref() {
                                            Val::Pi(name, icit, ty, closure) => {
                                                if *icit == Icit::Expl {
    ...
    fn trait_wrap(&mut self, cxt: &Cxt, t: Span<String>, a: Rc<Val>, x: Box<Raw>, tm: Rc<Tm>, t_span: Span<()>) -> Result<(Rc<Tm>, Rc<Val>), Error> {
        ...
        // 精化 σ 包裹的期望类型先 force 推开再判形态（旧机制下槽/类型已物化）
        let typ = self.force(&cxt.decl, &typ_raw).to_typ().unwrap_or(Typ::Val(empty_span("$unknown$".to_owned())));
        if t.data.is_empty() {
