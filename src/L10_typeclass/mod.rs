...
pub fn run(input: &str, path_id: u32) -> Result<String, Error> {
    let prelude = r#"
struct Tuple2[A: Type 0, B: Type 0] {
    _1: A
    _2: B
}

struct Tuple3[A: Type 0, B: Type 0, C: Type 0] {
    _1: A
    _2: B
    _3: C
}
"#;
    let prelude_ast = parser::parser(input, path_id).unwrap();
    let mut infer = Infer::new();
    let ast = parser::parser(input, path_id).unwrap();
    let mut cxt = Cxt::new();
    let mut ret = String::new();
    for tm in prelude_ast {
