...
//! （`NO_CONV_MEMO` 消融）+ 跨调用草稿（`ConvScratch`）+「清空按阈值归还
//! 缓冲」口径（`ReclaimOnClear` 与 `CACHE_SHRINK_MIN_ENTRIES`/
//! `SPINE_SHRINK_MIN_ENTRIES`）。原 bump_spine_iter.rs 的 "unify" 节，
//! 逐行搬运（2026-09-23 拆分）。
...
use super::machine::solve_probe;
use super::rename::{
    RenBuf, invert_bump, prune_meta_bump, solve_bump, solve_with_pren_bump,
};
