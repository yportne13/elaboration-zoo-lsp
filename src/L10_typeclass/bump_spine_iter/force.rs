//! force：迭代 force（臂集同 L09：Flex 展开 / Obj 重建 / VSub 推进）、显式
//! 替换推进（`frcs`/`frcs_env`）、深读点（`force_deep`）、参数视角 WHNF
...
thread_local! {
    pub(super) static PM_FUEL: std::cell::Cell<u32> = const { std::cell::Cell::new(UNIFY_FUEL) };
}
