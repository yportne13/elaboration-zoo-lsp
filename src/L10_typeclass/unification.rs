...
/// 一侧做 η 应用会命中 `v_app` 的 `impossible apply` panic——守卫失败直接
/// 落后续臂判失败（λ 与非函数值的比较无从展开，最小惊讶；快版 η 臂的
/// `vapp_ok` 同款守卫）。
        ...
        match (t.as_ref(), u.as_ref()) {
            ...
            (Val::Match(s1, env1, cases1), Val::Match(s2, env2, cases2)) => {
                ...
                for ((p1, clos1), (p2, clos2)) in cases1.iter().zip(cases2.iter()) {
                    ...
                    // 递归）——取代旧的整机克隆 + 就地改写 global 表
                    let body1_val = self.eval_neutral(&env1, clos1);
                    let body2_val = self.eval_neutral(&env2, clos2);
