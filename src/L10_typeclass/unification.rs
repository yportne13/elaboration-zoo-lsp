...
impl Infer {
    ...
    fn solve_trait(&mut self, cxt: &Cxt, m: MetaVar) -> Result<(), UnifyError>{
            ...
            if let Val::Sum(name, params, _, true) = &x {
                ...
                if let Some(a) = self.trait_solver.synth(Assertion {
                    ...
                    let (tm, _) = self.infer_expr(cxt, Raw::Var(name.clone().map(|_| format!("{:?}{:?}", a.name, a.arguments))))
                        .map_err(|e| UnifyError::Trait(e.0.data))?;
                    let val = self.eval(&cxt.env, tm);
                    println!("solve trait {:?}\nmeta: {}", val, idx + m.0 as usize + 1);
                    self.meta[idx + m.0 as usize + 1] = MetaEntry::Solved(val, x);
                } else {
                    Err(UnifyError::Trait(format!("solve trait failed: {:?}", params)))?
                }
            }
        }
        Ok(())
