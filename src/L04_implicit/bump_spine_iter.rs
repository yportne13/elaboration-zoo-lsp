...
/// 虽也以失败告终，但直接判失败与参考版守卫对齐，免去绕路。L04 无
    ...
    while let Some(item) = stack.pop() {
        ...
        match (v_tag(t), v_tag(u)) {
            ...
            (2, 2) => {
                ...
                // 受控内联环（L03 同款门控）：沿 `.a` 同步下走，仅在纯
                // ChainWrap 同头延续处（实参链顶层 `f` 与本层 `f` 同字）；
                // 实参若是另一条中性链（Apply 惯例：`f` = partial 句柄 ≠
                // 头字）则停下把子对交回完整分派——盲下钻会跳过内层头分派，
                // 把该走求解/intersect 的形态逐层 pairwise 误比。派发序与
                // 参考版 `unify_sp` 同序：停钻时先压实参对、后压函数部分对。
                let mut i1 = h1;
                let mut i2 = h2;
                loop {
                    let (f1, a1) = {
...
pub(crate) struct Machine {
    ...
    /// 生命周期重写指针类型（SAFETY：上述「进核即 clear」纪律 + 元素无 Drop，
    /// 使按当次 `'a` 重写指针类型安全，借出期条目不跨调用存活）。eval/quote/unify 共用
...
impl Machine {
    ...
    /// 每轮 reset：metacontext 清空 + 名字表/轨迹/环境区域清空 + 中性
    /// spine 清空（保容量）。tag-2 句柄只被上述各项与当轮 bump 值持有，
    /// 轮边界后无旧句柄可达；`ren` 存的是层级而非 spine 句柄（且 solve
    /// 入口换代），`vals`/`conv`/`quote_memo`/工作栈各在核函数入口 clear。
