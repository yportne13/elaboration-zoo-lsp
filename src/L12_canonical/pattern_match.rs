...
impl Compiler {
    ...
    > {
        ...
        // `meta_contrains` 由 `unify_catch` 入口/出口无条件清空，探测期挂账
        // 要么就地转 Err 要么被清，不持久；`mutable_map` 是 Rc 共享（整
        // Infer clone 亦共享），探测无全局登记；`hover_table`/
        // `completion_table` 只在 decl elaboration 追加。
