//! machine：稳态复用机（`Machine`）与 elaboration（check/infer/insert 族）、
//! Elaboration 上下文（`Cxt`/`TCons`/subst_cxt）、decl 表条目
//! （`DeclEntry`/`Decls`，原文件位于 σ 回收节尾，按内容归属）、trait 合成
//! 状态（`TraitState`/v_to_ref_val——本层无独立 typeclass 分节）、Debug
//! 复刻与 builtin 注册（prime_round/elab_all）。原 bump_spine_iter.rs 的
//! "Machine（稳态复用）与 elaboration"、"Elaboration 上下文"、"Debug 复刻"、
//! "builtin 注册" 各节及上述内容段，逐行搬运（2026-09-23 拆分）。
...
use super::env::{
    env_ext, env_ext_defs, env_len, env_nth, CloCell, Env, EnvCons, EMPTY_ENV, PiCell,
};
...
use super::spine::{is_flex, MetaEntry, MetaSnap, Spine};
use super::subst::{vsub_reclaim, wrap_sub, SpecSolve, SubstV};
use super::syntax::{
    LCons, PrimId, PrCons, SumDataT, SumParamT, SumParamV, Tm, V, XCell, v_clo,
    v_clo_of, v_lit_ty, v_lvl, v_lvl_of, v_meta, v_meta_of, v_pi, v_pi_of, v_spine_of,
    v_tag, v_u,
    v_u_of, v_xcell, v_xcell_of,
};
use super::unify::{
    unify_iter, CACHE_SHRINK_MIN_ENTRIES, ConvScratch, DECLB_CACHE, ReclaimOnClear,
    SPINE_SHRINK_MIN_ENTRIES, UItem,
};
use crate::L12_canonical::typeclass::{Assertion, Instance, Synth};
