//! force：force（迭代；臂集同 L11：Flex + Obj 两臂）+ frcs（显式替换推进
//! 值结构）/ frcs_env / force_deep / force_arg + val_mentions_lvl（原文件
//! 位于 σ 回收节内，按内容归属）+ vapp1/vapp_ok/project + 精化展开燃料
//! （PM_FUEL/refuel/burn；原文件位于 metacontext 节尾，按内容归属）。原
//! bump_spine_iter.rs 的 "force" + "frcs" 两节及上述内容段，逐行搬运
//! （2026-09-23 拆分）。
...
use super::syntax::{
    ...
    v_pi_of,
    v_spine_of, v_tag, v_xcell, v_xcell_of,
};
