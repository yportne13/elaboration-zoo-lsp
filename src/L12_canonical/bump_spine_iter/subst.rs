//! subst：模式特化的显式替换（`SubstV`/`SubEntryV`/`SUBSTV_ALIVE`）、σ
//! 克隆的跨轮回收（`VSUB_REGS`/`vsub_reclaim`）、读点包裹（`wrap_sub`）、
//! 浅扫描（`mentions_level`）与穿参状态（`SpecSolve`）。原
//! bump_spine_iter.rs 的 "values" 节尾的 SubstV 定义 + "σ 克隆的跨轮回收"
//! 节 + SpecSolve（原文件位于同节尾、env 段之前，按内容归属），逐行搬运
//! （2026-09-23 拆分）。
