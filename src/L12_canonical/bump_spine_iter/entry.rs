//! entry：对外入口——`export`（bump 项 → 参考 Box 树）、`tm_size`、稳态
//! 检查器 `Tycker`（run_decls/run_input/bench 口径）、`run_fast`/`parse`/
//! `SourceDecl`（Tycker 与入口四件套原文件位于 builtin 注册节内，按内容
//! 归属）。原 bump_spine_iter.rs 的 "export 与对外入口" 节及上述内容段，
//! 逐行搬运（2026-09-23 拆分）。
...
use crate::L12_canonical::Tm as CTm;

use super::env::env_len;
use super::force::refuel;
