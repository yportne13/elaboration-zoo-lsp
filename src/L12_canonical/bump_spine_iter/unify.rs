//! unify：工作表 unify（`UItem`/unify_iter + intersect/flex-flex/lockstep
//! + declb 存根表缓存 `DECLB_CACHE`/declb_of）+ ReclaimOnClear 清空归还
//! 口径与两个阈值常量（原文件位于本节头部，消费者为 unify/machine
//! 双侧）。原 bump_spine_iter.rs 的 "unify" 节，逐行搬运（2026-09-23 拆分）。
...
use super::machine::{DeclEntry, Decls};
use super::rename::{invert_bump, prune_meta_bump, RenBuf, solve_bump, solve_with_pren_bump};
use super::spine::{HK_OBJ, HK_OTHER, MetaEntry, Spine};
use super::syntax::{
    Tm, V, XCell, v_clo_of, v_lvl, v_lvl_of, v_meta_of, v_pi_of, v_spine_of, v_tag,
