...
//! （`MetaEntry`/`MetaSnap`/`meta_unsolved`/`meta_val_of`）及字面量取值
//! （`lit_of`）。原 bump_spine_iter.rs 的 "spine 栈（扁平中性）" +
//! "metacontext" 两节（含 metacontext 尾部的 lit_of），逐行搬运
//! （2026-09-23 拆分）。
