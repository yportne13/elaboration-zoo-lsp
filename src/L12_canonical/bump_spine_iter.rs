...
use super::{empty_span, Error, Ix, MetaVar, PatternDetail, Tm as CTm, cover_at};
...
impl SubstV {
    ...
    /// 包在解值外（解值不含 x 自身——occurs 守卫），由 force 在读点推开。
    /// 未命中 None。
    fn lookup_hit<'a>(
        bump: &'a Bump,
        spine: &Spine,
...
// --------------------------------------------------------------------------------
// σ 克隆的跨轮回收（2026-09-18，L07 README §7.7 的逐层传播）
// --------------------------------------------------------------------------------
...
fn wrap_sub<'a>(bump: &'a Bump, sub: &Rc<SubstV>, v: V) -> V {
    if sub.is_empty() {
        ...
        // Rc::from_raw + drop 会按未发生的 +1 过度递减引用计数（L07 评审 P2）
...
fn mentions_level(spine: &Spine, defs: &[V], v: V, sub: &SubstV) -> bool {
    ...
    match v_tag(v) {
        ...
        7 => match v_xcell_of(v) {
            XCell::Lit(_) | XCell::Prim { .. } | XCell::Decl { .. } => false,
...
/// 结构，闭包体 / Match 分支体跳过；**不扫 Flex 的 spine**（L12/L10 的元变量
/// 以全 scope 剪枝 spine 登记，spine 合法含当前方程的 rigid；旧 update_cxt
...
/// 特化合一的进行时状态（参考版 `SpecSolve` 同构）。L12 的可解集 = 任意
...
/// metacontext 条目（参考版 L12 三元同构）：**类型一律保留**（pruning 检查与
/// `lams` 都要读）；第二元 = 创建时刻的上下文快照（no_metas 错误路径）；
/// 第三元 = 创建时的开放类型（错误消息 pretty 与 oty 检查用）。解是 bump
/// 内的打包值。Clone 供 temp-infer 探测的快照换入换出。
#[derive(Clone)]
pub(crate) enum MetaEntry {
    Solved(V, V),
    Unsolved(V, Rc<MetaSnap<'static>>, V),
}

/// 取未解条目的（闭类型, 快照, 原始类型）。非未解 → unreachable。
#[inline]
pub(crate) fn meta_unsolved<'a>(m: &MetaEntry) -> (V, &'a MetaSnap<'a>, V) {
    match m {
        MetaEntry::Unsolved(a, snap, o) => {
...
fn meta_val_of(metas: &[MetaEntry], m: u32) -> V {
    match &metas[m as usize] {
        ...
        MetaEntry::Unsolved(v, _, _) => v_meta(m),
    }
}
...
fn vapp1<'a>(
    ...
    if v_tag(f) == 1 {
        ...
        match v_xcell_of(f) {
            ...
            // 把 VSub 头送进来——臂上下文里 Var 引用了被解槽），force 顶层
            // 不再产出 VSub，递归必终止
            XCell::VSub { .. } => {
                let f2 = force(bump, spine, defs, metas, decl, mutable, f);
                vapp1(bump, spine, work, vals, icits, defs, metas, decl, mutable, f2, a, i)
            }
...
/// fuel 是否已耗尽（参考版 `Infer::fuel_exhausted` 同款）：覆盖探测失败的
/// 降级方向判据——fuel 耗尽的失败按**可达**处理（保守要求覆盖）。
        ...
        match v_tag(v) {
            ...
            2 => {
                ...
                match &metas[v_meta_of(hd) as usize] {
                    MetaEntry::Unsolved(v, _, _) => return *v,
...
///
/// 本层无燃料池（与快版 force/unify 的既有权衡一致）：lookup 命中直接推进，
/// 不做有界降级。
#[allow(clippy::too_many_arguments)]
fn frcs<'a>(
    bump: &'a Bump,
    ...
    match v_tag(v0) {
        7 => match v_xcell_of(v0) {
            ...
            // 字面量 / 卡住声明（裸头）/ 卡住内建：σ 无从推进
            XCell::Lit(_) | XCell::Prim { .. } | XCell::Decl { .. } => {
        ...
        2 => {
            ...
            if v_tag(hd) == 0 {
                ...
                for &(u, i) in args.iter().rev() {
                    head = vapp1(
                        bump, spine, &mut work, &mut vals, &mut icits, defs, metas, decl, mutable,
...
/// `to_typ` / trait 求解等"结构消费者"的深读点（参考版 `Infer::force_deep`
/// 同款）：force 到 WHNF 后，Sum/SumCase 的槽位值也 force 推开（旧机制下
/// 这些槽位已被 update_cxt/refresh 物化）。只服务 trait 求解边界，不进
/// spine/闭包体。
fn force_deep<'a>(
    bump: &'a Bump,
    spine: &mut Spine,
...
/// 合一器**参数视角**的 WHNF（参考版 `Infer::force_arg` 同款）：逐层解包
/// VSub 后——裸 rigid 返回裸值；带实参的 rigid 链 / 卡住 match 返回**原值**
/// （包裹保留）；其余形态全量 force 推开。invert/prune_vflex 靠它保住
/// "元变量被应用在哪些槽位上"的可逆 spine。
fn force_arg<'a>(
    bump: &'a Bump,
    spine: &mut Spine,
    defs: &mut Vec<V>,
            ...
            QJob::Q(v0, level) => {
                ...
                match v_tag(v) {
                    ...
                    7 => match v_xcell_of(v) {
                        ...
                        // 不变式：QJob::Q 入口已 force（顶层不会是 VSub）；本层
                        // 无燃料降级例外，此臂仅防御——解包内层按同 level 再引
                        // （参考版 quote 的 VSub 臂同款，链式 VSub 由递归消化）
                        XCell::VSub { val, .. } => {
                            tasks.push(QJob::Q(*val, level));
                        }
                    },
...
/// (Obj, Obj) 合同臂用它做门控（L12 参考版有该臂；tag2 链的头种类在
...
/// 精化燃料：**已补齐**——frcs 的 lookup 命中与 Match 重选各烧 1
/// （thread_local `PM_FUEL` 4096，外部入口充值；耗尽时 lookup 命中降级为
/// 裸 rigid、Match 不重选），与参考版 `Infer::unify_fuel` 口径一致。
/// 另：L12 参考版 unify 的 `fuel` 参数只服务两处：Decl 头
/// 展开臂（`(Decl(..), _)` → quote+eval 后 fuel-1 重试）与 Match 分支体
/// `bind_cxt` 重锚的 SumCase 100 次配额——前者孪生侧 decl 表存根值
/// （`XCell::Decl`）无对应展开口径，机制缺口已登记已知分歧（无已知触发
/// 例）；`constraints`（参考版 `meta_contrains` 的 Stuck 挂账）已移植。
/// 【原注"无燃料/参考版无 fuel"有误：L12 参考版 unify 带 fuel，且 frcs
/// 燃料池已照 L09 孪生模板补齐——2026-09 连贯性评审订正。】另：无 pm 臂
/// （L09 的模式特化在 elaboration 侧的
...
fn invert_bump<'a>(
    ...
    for &(a, _) in args.iter().rev() {
        // 应用序（外先）。参数视角 WHNF：精化 σ 只解包不展开（物化会把
            ...
            RJob::Ren { dom, cod, v } => {
                ...
                match v_tag(v) {
                    ...
                    7 => match v_xcell_of(v) {
                        ...
                        // 不变式：入口已 force（顶层不会是 VSub）；防御臂按
                        // 参考版 rename 的 VSub 臂同款判失败
...
impl Machine {
    ...
    fn new_meta<'a>(&mut self, bump: &'a Bump, cxt: &Cxt<'a>, a: V, origin_typ: V) -> u32 {
        ...
        self.metas.push(MetaEntry::Unsolved(a, snap, origin_typ));
    ...
    fn fresh_meta<'a>(&mut self, bump: &'a Bump, cxt: &Cxt<'a>, a: V) -> &'a Tm<'a> {
        ...
        if is_trait_sum {
            return bump.alloc(Tm::Meta(self.new_meta(bump, cxt, a, a)));
        }
    ...
    fn solve_multi_trait_ref<'a>(
        ...
        let prepare: Vec<(u32, V)> = self
            ...
            .flat_map(|(i, x)| match x {
                MetaEntry::Unsolved(v, _, _) => Some((i as u32, *v)),
                _ => None,
            })
            .collect();
    ...
    /// 参考 `Infer::solve_trait`（L12）：x 是 trait Sum → 实参 force 成
    /// Val 直通车（**任一仍是 Flex 即 Ok(None)** 交给合一，参考版同款）；
    /// 命中给 (实例项, 实例值)，失败给 Err 文案，非 trait 给 None。
    fn solve_trait_ref<'a>(
        ...
        // 实参：force 后逐项解码成参考版 Val（trait 求解器在 Val 级匹配）
        let forced: Vec<V> = {
            let Machine {
                spine, defs, metas, mutable, ..
            } = self;
            params
                .iter()
                .zip(out_param.iter())
                ...
                .map(|(p, _)| force_deep(bump, spine, defs, metas, &*cxt.decls, mutable, p.val))
                .collect()
        };
        // 任一实参仍是 Flex（未解 meta）→ 交给合一（参考版同款短路）。
        // `is_flex` 同时覆盖裸 meta（tag 5）与 meta 头链（`?m x`，tag 2 的
        // HK_FLEX 头）——参考版 `Val::Flex(..)` 含 spine，本就两者都拦
        // （L13 40199ef 同族）。
        if forced.iter().any(|f| is_flex(&self.spine, *f)) {
            return Ok(None);
        }
        let args_val: Vec<Arc<CVal>> = forced
            .into_iter()
            .map(|f| v_to_ref_val(&self.spine, &self.defs, f))
            .collect();
        ...
        if let Some(a) = answer {
            ...
            if v_tag(val) == 7 {
                if let XCell::SumCase { typ, .. } = v_xcell_of(val) {
                    // 参考版：unify_catch(typ, x) 把关（结果弃置）
                    let _ = self.unify_catch(bump, cxt, *typ, x);
                }
            }
            Ok(Some((tm, val)))
        } else {
            // 参考版文案：`{}[{:?}]`（Val Debug）+ 类实例表逐行
            let params_dbg = args_val
                .iter()
                .map(|v| format!("{:?}", v))
                .collect::<Vec<_>>()
                .join(", ");
...
/// `subst sub ctx`）：env 槽、名字表的类型值（`names.by_lvl`，参考版
/// `src_names` 的 map2）与 `types` 链的类型值包 `VSub`；lvl / locals /
/// pruning / binds / decls 不动——**槽位布局（= 运行时布局）不变**，被解
/// 变量仍在原槽位，读点经 force 推开看到解。σ 为空时零开销直通。
///
/// 替代旧的 `update_cxt`/`refresh`（改写 env 槽 + 全量重引用）：解不再改写
/// 既有值，只包在外面，消费点惰性展开——旧架构"已捕获旧上下文的值过期、
/// 槽位错位"的 bug 族按构造消除。
...
impl Machine {
    ...
    /// `unify_pm`：模式特化的合一（参考版 elaboration.rs `unify_pm` 同款
    /// 臂序与口径）：双裸 Rigid 同级自反；单侧裸 Rigid → 解**累加进
    /// `spec.acc`**（显式替换，不再改写环境）；同名 SumCase 逐 datas、同名
    /// Sum 逐参数（均**只比值槽**）递归；其余落 `unify_catch`（全文案合一
    /// 错误）。方程两侧在入口置于当前 acc 之下再解释（`subst ɑ vs` 的惰性
    /// 等价物）。
    ///
    /// 与 canonical / trait 求解的交互：本函数**只**服务模式方程路径
    /// （pattern_match 的 `check_pm`/`check_pm_final`）；canonical 搜索在
    /// 快版不移植，trait 实例求解（`solve_trait_ref`）走常规 unify，不带
    /// spec——实例求解过程中的合一**不会**获得特化解能力（对齐旧
    ...
    fn unify_pm<'a>(
        ...
        t = self.force_v(bump, cxt, t);
        t_prime = self.force_v(bump, cxt, t_prime);
        if !spec.acc.is_empty() {
            t = self.force_v(bump, cxt, wrap_sub(bump, &spec.acc, t));
            t_prime = self.force_v(bump, cxt, wrap_sub(bump, &spec.acc, t_prime));
        }
        // (Rigid(x1, []), Rigid(x2, [])) if x1 == x2 → 自反（旧
        // update_cxt(x1, t_prime, update_prune=false) 是恒等，语义为 Ok）
        if v_tag(t) == 0 && v_tag(t_prime) == 0 && v_lvl_of(t) == v_lvl_of(t_prime) {
            return Ok(());
        }
        // (Rigid(x, []), v) → 解 x := v 入 acc。Flex 解值保持旧 update_cxt
        // 的 no-op；occurs 环守卫只扫解值自身结构（Flex 头不透明），失败 =
        // 方程不可满足（分支不可达）。
        if v_tag(t) == 0 {
            let x = v_lvl_of(t);
            if is_flex(&self.spine, t_prime) {
                return Ok(());
            }
            if val_mentions_lvl(&self.spine, &self.defs, t_prime, x) {
                return Err(Error(t_span.map(|_| "".to_string()), vec![]));
            }
            spec.acc = SubstV::extend(&spec.acc, x, t_prime);
            return Ok(());
        }
        // (v, Rigid(x, [])) → 解 x := v
        if v_tag(t_prime) == 0 {
            let x = v_lvl_of(t_prime);
            if is_flex(&self.spine, t) {
                return Ok(());
            }
            if val_mentions_lvl(&self.spine, &self.defs, t, x) {
                return Err(Error(t_span.map(|_| "".to_string()), vec![]));
            }
            spec.acc = SubstV::extend(&spec.acc, x, t);
            return Ok(());
        }
        // 同名 SumCase：逐 datas（值槽）
        if v_tag(t) == 7 && v_tag(t_prime) == 7 {
            if let (
                XCell::SumCase { typ: ty1, case_name: n1, datas: d1, .. },
                XCell::SumCase { typ: ty2, case_name: n2, datas: d2, .. },
            ) = (v_xcell_of(t), v_xcell_of(t_prime))
            {
                if n1 == n2 {
                    // **比 Sum 头名字**，不比 typ 的值（参考版 unify_pm 同款
                    // 2026-09-18）：跨 enum 重名构造子（E1.c / E2.c）是两个
                    // 不同值，同 case_name 不足以判定身份——头名不同直接失
                    // 败。typ 的值的比较会经索引槽互相引用深递归。两侧 force
                    // 后任一非 Sum（如未解 meta）则不比。
                    let (t1f, t2f) = (self.force_v(bump, cxt, *ty1), self.force_v(bump, cxt, *ty2));
                    if let (
                        XCell::Sum { name: h1, .. },
                        XCell::Sum { name: h2, .. },
        ...
        self.unify_catch(bump, cxt, t, t_prime)
    }
    ...
    /// `check_pm`：infer + insert + `unify_pm`（特化解累积为显式替换 σ）。
    fn check_pm<'a>(
        &mut self,
        bump: &'a Bump,
        cxt: &Cxt<'a>,
    ...
    /// `check_pm_final`：`check_pm` 之后把原始值与精化后的模式项值再对一次
    /// （旧 `.unwrap_or(new_cxt)`——失败容忍，丢弃第二次方程的解）。第二方程
    /// 的上下文置于 σ 之下（`subst_cxt` 包裹 ⇒ eval 出来的引用带 VSub，
    /// unify_pm 入口统一推开）。
    fn check_pm_final<'a>(
        &mut self,
        bump: &'a Bump,
        ...
        let ori_v = self.eval(bump, cxt, cxt2.env, t_inferred);
        let mut spec2 = SpecSolve {
            acc: spec.acc.clone(),
        };
        if self
            .unify_pm(bump, &cxt2, ori, ori_v, &t_span, &mut spec2)
            .is_ok()
        {
            spec.acc = spec2.acc;
        }
    ...
    fn trait_wrap<'a>(
        ...
        let typ = v_to_ref_val(&self.spine, &self.defs, a);
        // 方法名命中的 trait：合成宽通配参数的 Assertion（首参 = 接收者
        // 类型，其余 Flex(u32::MAX) 通配）——能解出实例才包装
        let defs: Vec<(
            SmolStr,
            Vec<(crate::parser_lib::Span<SmolStr>, Raw, Icit)>,
            Vec<bool>,
            (crate::parser_lib::Span<SmolStr>, Vec<(crate::parser_lib::Span<SmolStr>, Raw, Icit)>, Raw),
        )> = self
            .tstate
            .definition
            .iter()
            ...
            .filter(|(trait_name, _, out_param, _)| {
                ...
                let wildcard: Arc<CVal> = Arc::new(CVal::Flex(super::MetaVar(u32::MAX), CList::new()));
                let mut args = vec![wildcard.clone(); len];
                args[0] = typ.clone();
                self.tstate.solver.clean();
                self.tstate
                    .solver
        ...
        for (trait_name, trait_params, _out_param, (methods_name, methods_params, ret_type)) in defs {
            ...
            params.push((
                ...
                trait_params
                    ...
                    .map(|x| x.0.clone())
                    .fold(
                        Raw::Var(methods_name.clone().map(|_| SmolStr::new(&trait_name))),
            ...
            let body = std::iter::once((
                Raw::Var(methods_name.clone().map(|_| SmolStr::new("$this"))),
                Icit::Expl,
            ))
        ...
        match d {
            ...
            Decl::Enum {
                    ...
                    cases.iter().zip(new_cases.iter())
                {
                    ...
                    let vtyp = self.eval(bump, &cxt, cxt.env, typ_tm);
                    // 构造子良构性（参考版同步）：ret 必须是本 enum 的 Sum
                    // 且参数位是 telescope 内的 bare rigid
                    self.check_ctor_wf(
                        bump,
                        &cxt,
                        &name.data,
            ...
            Decl::TraitDecl { name, params, methods } => {
                ...
                let mut param = vec![(name.clone().map(|_| SmolStr::new("Self")), Raw::Hole(empty_span(())), Icit::Impl)];
                ...
                let new_cases = vec![(
                    ...
                    methods
                        ...
                        .map(|x| (
                            x.0.clone(),
                            std::iter::once((x.0.clone().map(|_| SmolStr::new("this")), Raw::Var(x.0.clone().map(|_| SmolStr::new("Self"))), Icit::Expl))
            ...
            Decl::ImplDecl { name, params, trait_name, trait_params, methods, need_create } => {
                ...
                let typ = v_to_ref_val(&self.spine, &self.defs, typ_val);
                let mut trait_param = vec![typ.clone()];
                for a in trait_params.iter() {
                    let (a_checked, _) = self.check_universe(bump, &cxt, a)?;
                    let a_eval = self.eval(bump, &cxt, cxt.env, a_checked);
                    trait_param.push(v_to_ref_val(&self.spine, &self.defs, a_eval));
                }
                ...
                let trait_param: Vec<Arc<CVal>> = trait_param.into_iter()
                    .zip(out_param.iter())
                    .filter(|(_, o)| !**o)
                    .map(|(x, _)| x)
                    .collect();
                ...
                let inst = Instance {
                    ...
                    lvl: trait_name.clone().to_span().map(|_| SmolStr::new(typ_name.clone())),
                };
                ...
                for decl in methods {
                    if let Decl::Def { name: def_name, params, ret_type: _, body } = decl {
                        ret = Raw::App(
                            Box::new(ret),
                            Box::new(Raw::Lam(
                                def_name.clone().map(|_| SmolStr::new("this")),
                                Either::Icit(Icit::Expl),
                ...
                let def_name = trait_name.clone().to_span().map(|_| SmolStr::new(typ_name.clone()));
    ...
    /// 2026-09-18 前向传播）：实例化构造子类型的全部绑定器后，ret 的 WHNF
    ...
    fn check_ctor_wf(
        ...
        let retf = self.force_v(bump, cxt, ty);
        if !(v_tag(retf) == 7 && matches!(v_xcell_of(retf), XCell::Sum { .. })) {
            return Err(Error(span.map(|_| {
...
pub(crate) struct TraitState {
    ...
    definition:
        HashMap<SmolStr, (Vec<(crate::parser_lib::Span<SmolStr>, Raw, Icit)>, Vec<bool>, Vec<(crate::parser_lib::Span<SmolStr>, Vec<(crate::parser_lib::Span<SmolStr>, Raw, Icit)>, Raw)>)>,
...
/// meta 立即数）；链解码为"宽放"占位（实参逐个包成 always-match 的
/// Flex 链——参考版 `Val::Flex` 含 spine 同款宽放），其余形态按同口径
/// 宽放，不再 unreachable panic。
pub(crate) fn v_to_ref_val(spine: &Spine, defs: &[V], v: V) -> Arc<CVal> {
    ...
    use std::sync::Arc as SrcRc;
    match v_tag(v) {
        0 => SrcRc::new(CVal::Rigid(super::Lvl(v_lvl_of(v)), CList::new())),
        2 => {
            // 链：头是裸变量 → Rigid + spine args（解码成 reference 形态）
            let h = v_spine_of(v);
            let hd = spine.spine_head(h);
            ...
            for (a, i) in args.iter() {
                acc = SrcRc::new(CVal::Flex(super::MetaVar(u32::MAX), CList::new().prepend((v_to_ref_val(spine, defs, *a), *i))));
            }
            acc
        }
        3 => SrcRc::new(CVal::U(v_u_of(v))),
        5 => SrcRc::new(CVal::Flex(super::MetaVar(v_meta_of(v)), CList::new())),
        6 => SrcRc::new(CVal::LiteralType),
        7 => match v_xcell_of(v) {
            XCell::Lit(s) => SrcRc::new(CVal::LiteralIntro(crate::parser_lib::Span {
                data: s.to_string(),
                start_offset: 0,
                end_offset: 0,
                path_id: 0,
            ...
            XCell::Sum {
                ...
                let mut ps: Vec<(crate::parser_lib::Span<SmolStr>, SrcRc<CVal>, SrcRc<CVal>, Icit)> =
                ...
                SrcRc::new(CVal::Sum(nspan(*name), ps, cs, *is_trait))
            }
            XCell::SumCase {
                typ,
                case_name,
                ...
                let mut ds: Vec<(crate::parser_lib::Span<SmolStr>, SrcRc<CVal>, Icit)> =
                    Vec::with_capacity(datas.len());
                for d in datas.iter() {
                    ds.push((nspan(d.name), v_to_ref_val(spine, defs, d.val), d.icit));
                }
                SrcRc::new(CVal::SumCase {
                    is_trait: *is_trait,
                    typ: v_to_ref_val(spine, defs, *typ),
                    case_name: nspan(case_name),
                    datas: ds,
                })
            }
            // Pi / Match / Decl 链等形态：参考版 val_match 对非构造子
            // goal 一律 false——降级为永不匹配的标记值，观察面相同。
            _ => SrcRc::new(CVal::LiteralIntro(crate::parser_lib::Span {
...
fn export(t: &Tm<'_>) -> CTm {
    ...
    while let Some(j) = tasks.pop() {
        match j {
            ...
            J::Do(Tm::LiteralIntro(s)) => done.push(CTm::LiteralIntro(
                crate::parser_lib::Span {
                    data: s.to_string(),
                    start_offset: 0,
                    end_offset: 0,
            ...
            J::Sum2 {
                ...
                let mut ps: Vec<(crate::parser_lib::Span<SmolStr>, CRc<CTm>, CRc<CTm>, Icit)> =
                    Vec::with_capacity(params.len());
                for p in params.iter().rev() {
            ...
            J::SumCase2 {
                ...
                let mut ds: Vec<(crate::parser_lib::Span<SmolStr>, CRc<CTm>, Icit)> =
                    Vec::with_capacity(datas.len());
                for d in datas.iter().rev() {
...
fn no_metas<'a>(
    ...
    cxt: &Cxt<'a>,
    t: &'a Tm<'a>,
) -> Option<(Rc<MetaSnap<'static>>, V)> {
    let mut stack: Vec<&Tm<'a>> = vec![t];
    while let Some(x) = stack.pop() {
        match x {
            Tm::Meta(mm) => match &m.metas[*mm as usize] {
                MetaEntry::Unsolved(_, snap, oty) => {
                    return Some((snap.clone(), *oty));
                }
...
fn err_unsolved_meta<'a>(
    ...
    if is_trait_sum {
        ...
        let instances: Option<Vec<SmolStr>> = m
            .tstate
            .solver
            .class_instances
            .get(name)
