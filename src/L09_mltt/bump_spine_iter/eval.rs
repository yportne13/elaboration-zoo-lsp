...
//! eval（`W`/`eval_iter`，右链快速路径 + AppPruning + L09 变体）。原
//! bump_spine_iter.rs 的 "运行时分支选择" + "eval" 两节，逐行搬运（2026-09-23 拆分）。
...
use super::env::{CloCell, Env, PiCell, env_ext, env_nth};
use super::force::{force, vapp1};
use super::spine::{MetaEntry, Spine, lit_of, meta_val_of};
use super::syntax::{GLOBAL_BASE, PrCons, SumDataT, SumDataV, SumParamT, SumParamV, Tm, V, XCell, v_clo, v_clo_of, v_lit_ty, v_pi, v_spine_of, v_tag, v_u, v_xcell, v_xcell_of};
