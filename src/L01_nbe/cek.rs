...
//! 同格式，Lam 长度占位回填），解码是前向扫描 + 组装循环——`Clo` 分支复用
//! 同一个**迭代** `eval`（顺序调用，不产生原生递归），求值/quote 主循环本身
//! 不吃机器栈。
//!
//! 收益：**求值深度不受进程栈限制**——递归变体在 n ≥ 16000 时构造/求值/
//! 比较全链路爆栈，`cek` 可以一路跑到 n = 51 万（readme 有实测）。代价：
//! 每步多一次显式栈压弹和宽分派，小规模上比递归版慢约 10×；要速度可换
//! `cek_bump`（bump 分配）或 `bump_iter`（双栈，同为栈安全、只慢 1.2×）。
//!
//! 已知残余栈消耗（定位结论，见 readme"已知限制"）：`Value` 的派生
//! `Clone`/`Drop` 是递归的——`Idx` 查表克隆环境里的 `Clo` 会对闭包体
//! （`Box<Term>` 深树）逐层深拷贝，深中性链析构同理。求值循环本身不吃
//! 机器栈，大规模靠 bench 的大栈线程兜底；教学定位，不做 Rc 化重构。
