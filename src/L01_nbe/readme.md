...
| n | cek min ms | 扩展 |
|---|---|---|
| n | cek ms | cek_bump ms | 相对提速 |
|---|---|---|---|
| 8000 | 2.919 | 0.331 | 8.8× |
| 16000 | 5.94 | 0.96 | 6.2× |
| 32000 | 12.10 | 1.62 | 7.5× |
| 64000 | 25.61 | 3.60 | 7.1× |
| 128000 | 60.03 | 8.82 | 6.8× |
| 256000 | 124.34 | 32.04 | 3.9× |
| 512000 | 252.20 | 67.34 | 3.7× |
...
6. **`cek` 是最慢的常规变体（~11×）**——这个"慢"已被第五轮解决：**`cek_bump`
   （CEK 迭代 eval + bump 全分配/结果 bump）实现速度与深度兼得**——小 n 仅比
   递归冠军 `bump_tree` 慢 ~20%（8000 时 0.331 vs 0.277ms），深度无上限
   （512000 时 67ms 线性扩展，比 `cek` 快 3.7×）。这是栈安全方向的最终答案：
   **显式 kont 栈的代价从 ~11× 压缩到 ~1.2×，换来深度不受进程栈限**。
   （注意：51 万+规模的线性栈消耗点尚未定位——栈指针采样证明不在 quote 主
   循环；l01bench 用大栈线程跑大规模，默认 1MB 栈下 25.6 万可稳定通过。）
