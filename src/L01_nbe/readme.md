...
† `compiled` 行是 2026-09 复测值：修复前它的中性应用比 `bump_tree` 多一次
bump 分配（2 次 vs 1 次），即"只差项访问一层"的对照混入了一个分配变量，
旧值 0.184 / 0.349 因而偏低、已作废。复测同轮基线 `bump_spine_iter_ss` =
0.030（4000）/ 0.060（8000），单轮 7 次（本轮机器噪声约 ±10-15%）。
...
- **原生闭包轴被封顶**（`native_clo`，0.084 ms，慢 2.8×）：项编译为
  ...
  它比 `compiled`（指令数组解释，0.220）快 ~2.6×——证明"编译"方向
