...
| env_slice | 0.185 | 0.190 | 1.21×（第七轮实测） |
...
| env_slice | ~0.35 | 1.27×（第七轮实测，统计持平） |
...
7. **第七轮：bump_tree 的三个指定优化项实测**（Bv 实现 Copy；apply_val 的
   两次 `bump.alloc` 合并为一次 `alloc([vf, va])`；环境改 bump 数组切片
   → `env_slice` 变体）：
   - `Bv` 已是全指针字段，`#[derive(Clone, Copy)]` 化性能无可见变化（编译器
     本就把 clone 内联为位拷贝），收益在语义简洁与切片操作免调用点；
   - 合并 `alloc([vf, va])`：单次对齐/检查/指针推进 + 两个子值相邻存放，
     微收益，保留；
   - `env_slice`（`nth` 变 O(1) 下标）：与 `bump_tree` 统计持平（4000 时
     0.185 vs 0.200 略优，8000 两轮 ±7% 互有胜负）——O(1) 查表被 prepend
     复制成本（每步写 4 个值 vs 链表 1 个节点）抵消。保留价值在**深索引
     负载**（de Bruijn 索引大时链遍历 O(idx) 会拖垮链表版），当前教堂数
     基准的索引 ≤ 2 测不出差异。

8. 还没试过的候选中，唯一可能再往下的路子是算法层的形态融合（在展开的
