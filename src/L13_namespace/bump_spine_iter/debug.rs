//! debug：错误消息里内嵌 `{:?}` 输出的 Debug 复刻（`dbg_span_str`/
//! `debug_val`/`debug_spine`/`debug_tm`/`debug_pat_go` 等；名字 Span 全零
//! ——套件比对前按 start_offset/end_offset/path_id 归一化）。原
//! bump_spine_iter.rs 的 "Debug 复刻" 节，逐行搬运（2026-09-23 拆分）。
...
use super::syntax::{
    Tm, V, XCell, v_clo_of, v_lvl_of, v_meta_of, v_pi_of, v_spine_of, v_tag, v_u_of,
    v_xcell_of,
};
