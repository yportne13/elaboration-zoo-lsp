//! quote：任务栈迭代 quote（`QJob`/`quote_iter` + `QuoteMemo` 记忆化）与
//! 原生 Nat 的项层展开（`quote_nat_chain`）。原 bump_spine_iter.rs 的
//! "quote" 节（"unify" 节标题之后的 `quote_nat_chain` 一并归属本模块），
//! 逐行搬运（2026-09-23 拆分）。
...
use super::force::{force, QUOTE_NAT_TM};
use super::machine::v_u0;
