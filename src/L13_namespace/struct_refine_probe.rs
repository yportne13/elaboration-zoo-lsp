...
fn summary() {
    ...
    println!();
    println!("薄弱点 (当前状态)：");
    println!("  weak01_direct_return_bound_tail    FAIL — 最简复现 (正确拒绝：确实类型错误)");
    println!("  weak02_struct_return_bound_tail    FAIL — 单层 struct + 泛型 len (正确拒绝)");
    println!("  weak03_two_fields_dependent        FAIL — 两字段交叉索引依赖 (仍需修复)");
    println!("  weak04_nested_generic_return_tail  FAIL — 嵌套 struct + 泛型 len (正确拒绝)");
    println!("  weak05_vecholder_recursive         FAIL — VecHolder 递归 (仍需修复)");
    println!();
    println!("修复进展 (elaboration.rs + cxt.rs)：");
    println!("  ✓ unify_pm: 不同 Rigid 级别时优先更新未细化的变量，保留已有细化");
    println!("  ✓ infer_expr(Raw::Var): 在细化环境中重求值变量类型 (quote+eval)");
    println!("  ✓ cxt.is_refined(): 提供公共 API 检查环境是否被细化过");
    println!();
    println!("仍开放的薄弱点 (deep fix required)：");
    println!("  跨字段索引依赖——check_pm_final 的模式推断创建独立元变量，");
    println!("  不与 Pi 循环中建立的 Rigid 绑定联通；需要改变 new_heads 中");
    println!("  Impl 参数的变量类型 (用元变量代替自我引用 Rigid)。");
    println!("----------------------------------------");
    println!();
}
