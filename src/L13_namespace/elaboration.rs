    ...
    pub fn check<const CANONICAL: bool>(&mut self, cxt: &Cxt, t: Raw, a: &Rc<Val>) -> Result<Rc<Tm>, Error> {
        ...
        match (t, a.as_ref()) {
            ...
            (Raw::Let(x, ret_typ, t, u), _) => {
                ...
                self.hover_table.push((x.to_span(), (None, x.to_span()), cxt.clone_without_src_names(), va.clone()));
    ...
    pub fn infer_expr(&mut self, cxt: &Cxt, t: Raw) -> Result<(Rc<Tm>, Rc<Val>), Error> {
        ...
        match t {
            ...
            Raw::Var(name) => match cxt.src_names.get(&name.data) {
                Some((x, a)) => {
                    self.hover_table.push((t_span, (None, a.0), cxt.clone_without_src_names(), a.1.clone()));
                    Ok((Tm::Var(lvl2ix(cxt.lvl, *x)).into(), a.1.clone()))
                },
                None => match cxt.decl.get(&name.data) {
                    Some((def, _, _, _, vty)) => {
                        self.hover_table.push((t_span, (self.decl_source.get(&name.data).cloned(), *def), cxt.clone_without_src_names(), vty.clone()));
                    ...
                    None => {
                        ...
                        self.hover_table.push((t_span, (self.decl_source.get(&qualified).cloned(), *def), cxt.clone_without_src_names(), vty.clone()));
                        ...
                        if let Some((full_key, (def_span, _, _, _, vty))) = match_entry {
                            self.hover_table.push((t_span, (self.decl_source.get(&full_key).cloned(), def_span), cxt.clone_without_src_names(), vty.clone()));
...
self.hover_table.push((t_span, (self.decl_source.get(&qual).cloned(), *def_span), cxt.clone_without_src_names(), vty.clone()));
...
self.hover_table.push((t_span, (self.decl_source.get(&prefixed).cloned(), *def_span), cxt.clone_without_src_names(), vty.clone()));
                ...
                match (tm, a.as_ref()) {
                    (tm, Val::Sum(_, params, cases, _)) => {
                        ...
                        if let Some((def_span, val)) = field_info {
                            self.hover_table.push((t.to_span(), (None, def_span), cxt.clone_without_src_names(), val.clone()));
                            Ok((Tm::Obj(tm, t).into(), val))
                        } else {
